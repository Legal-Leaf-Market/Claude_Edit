 ...
 *
 * "system" survives as a real state you can choose. It just is not what you
 * get by default any more, which means it has to be STORED: absence means
 * dark, so "system" cannot be represented by clearing the key.
 */

export const THEME_STORAGE_KEY = "stompbox-theme"
 ...
 * too late and the page flashes the wrong theme for a frame. Wrapped in
...
export function applyTheme(choice: ThemeChoice): void {
  ...
  try {
    // Every choice is stored, "system" included. Clearing the key now means
    // dark rather than system, so a removeItem shortcut here would silently
    // turn "follow my OS" into "dark".

 ...
 *
 * SCOPED TO THE GUIDE'S OWN TREE, NOT THE WHOLE REPO, and that is a real limit
 * worth stating rather than leaving to be discovered. The em dash rule is a
 * HOUSE rule (CLAUDE.md section 17) and it covers Gear Avail too, but this
 * walk cannot be the thing that enforces it there: pointed at the repo root it
 * flags nineteen files, and most of them are code that HANDLES em dashes
 * rather than copy containing one, including `model-parse.ts` normalising them
 * out of feed titles and three sibling tests asserting their absence. A test
 * that checks for an em dash has to contain an em dash. Widening this walk
 * means teaching it to tell copy from code, which is a different job than the
 * one it does well here.
 */
const SEARCH_DIRS = [
  "app/stompbox",
  "components/stompbox",

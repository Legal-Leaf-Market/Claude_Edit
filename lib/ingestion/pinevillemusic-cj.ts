...
/**
 * Pineville Music ingestion, via their CJ Affiliate product feed.
 *
 * Same CJ network and standard field set as zZounds and Full Compass Systems
 * (lib/ingestion/zzounds-cj.ts, lib/ingestion/fullcompass-cj.ts). No-ops on an
 * unset feed URL. Does not scrape pinevillemusic.com.
 */
...
export function normalizePinevilleMusicRow(row: Record<string, string>): NewMarketplaceListing | null {
...
export async function fetchPinevilleMusicFeedText(
  url: string,
...
export function parsePinevilleMusicFeed(text: string): {
...
export type PinevilleMusicIngestOutcome = {
...
export async function ingestPinevilleMusicFeed(
  fetchImpl: typeof fetch = fetch,
): Promise<PinevilleMusicIngestOutcome> {
  ...
  if (!env.cj.hasPinevilleMusicFeed) {
    const reason =
      "CJ_PINEVILLEMUSIC_FEED_URL is not set. Pineville Music ingestion is skipped. " +
      "Confirm the feed URL in the CJ Affiliate account dashboard under the Pineville Music programme."
    console.warn(`[pinevillemusic] ${reason}`)
    return { status: "skipped", reason, stats: empty, resolved: 0 }
  }

  const run = await startRun("pinevillemusic", "pinevillemusic-feed")

  try {
    const text = await withBackoff(
      () => fetchPinevilleMusicFeedText(env.cj.pinevilleMusicFeedUrl, fetchImpl),
    ...
    await expirePastEndDate("pinevillemusic")
    ...
    console.error(`[pinevillemusic] failed: ${message}`)

...
export const MERCHANTS: MerchantRow[] = [
  ...
  { merchant: "Zoro", platform: "LinkConnector", ratePct: null, status: "pending", catalogueCount: 0, note: "Campaign application SUBMITTED 10 Aug 2026, and submitted under nicotiamarket.com rather than gearavail.com, so approval does not by itself license Zoro links on this site. The ONLY row here that is not paid as a percentage of a sale: the campaign pays a variable CLICK commission of up to $0.36 on product links carrying lc_pid, with no sale required and no cookie window. ratePct stays null because a per-click rate cannot be expressed as one, which also means this merchant's revenue is invisible to the engine's commissionPct model and has to be reasoned about separately. Catalogue is an industrial supply feed of ~3.5M SKUs filtered down to its musical-instrument branch, so expect a catalogueCount in the hundreds or low thousands, weighted towards accessories, not a music retailer's full range" },

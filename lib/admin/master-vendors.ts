...
const SISTER_VENDORS: Record<string, MasterVendorRow[]> = {
  ...
  nicotia: [
    ...
    { merchant: "Black Buffalo", platform: "Refersion", ratePct: 10, status: "pending", sharePct: 0, note: "Confirmed 10 Aug 2026: the whole Refersion ACCOUNT is pending, not just this programme, and Black Buffalo is currently the only Refersion merchant across all five sites, so the account approval and this row's revenue are the same event. Direct outreach to Black Buffalo sent 10 Aug 2026, reply expected within days. Unlike Awin or CJ, Refersion programmes are administered by the merchant rather than vetted centrally by the network, so merchant advocacy is a plausible route to clearing an account-level pending state rather than a parallel conversation. Earns nothing until it clears" },
  ],
}
